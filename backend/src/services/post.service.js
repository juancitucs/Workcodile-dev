...
const postAggregationPipeline = [
  ...
  {
    $project: {
      title: 1, content: 1, course_id: 1, createdAt: 1, updatedAt: 1,
      hashtags: 1, attachments: 1, views: 1, upvote_count: 1, downvote_count: 1,
      upvoted_by: 1, downvoted_by: 1,
      author: { _id: '$author._id', name: '$author.name', avatar_key: '$author.avatar_key', level: '$author.level' },
      comments: {
        $filter: {
          input: '$comments',
          as: 'comment',
...
function addUserVoteStatus(item, currentUserId) {
  if (!currentUserId) { item.user_vote = null; return }
  const uid = new ObjectId(currentUserId)

  if (item.upvoted_by && item.upvoted_by.some(id => id.equals(uid))) {item.user_vote = 'up'}
  else if (item.downvoted_by && item.downvoted_by.some(id => id.equals(uid))) {item.user_vote = 'down'}
  else {item.user_vote = null}

  if (item.comments) {
    item.comments.forEach(comment => {
      addUserVoteStatus(comment, currentUserId)
      if (comment.replies) {comment.replies.forEach(r => addUserVoteStatus(r, currentUserId))}
    })
  }
}
...
function addUrlsToItems(items) {
  ...
  for (const item of items) {
    if (item.author && item.author.avatar_key) {item.author.avatar = getFileUrl(item.author.avatar_key)}
    if (item.attachments) {item.attachments.forEach(att => { if (att.object_key) {att.url = getFileUrl(att.object_key)} })}
    if (item.comments) {addUrlsToItems(item.comments)}
    if (item.replies) {addUrlsToItems(item.replies)}
  }
}
...
function sortComments(comments) {
  ...
  for (const c of comments) { if (c.replies) {sortComments(c.replies)} }
}

function findCommentRecursive(comments, targetCommentId) {
  for (const comment of comments) {
    if (comment._id.equals(targetCommentId)) {return comment}
...
async function populateAuthors(comments) {
  for (const comment of comments) {
    let authorId = null
    if (comment.author && comment.author._id) {authorId = new ObjectId(comment.author._id)}
    else if (comment.author) {authorId = new ObjectId(comment.author)}

    if (authorId) {
      const author = await User.findById(authorId)
      if (author) {
        comment.author = { _id: author._id, name: author.name, avatar_key: author.avatar_key, level: author.level || 1 }
      } else {
    ...
    if (comment.replies && comment.replies.length > 0) {await populateAuthors(comment.replies)}
  }
}
...
async function createPostDocument(data) {
  ...
  const newPost = {
    title, content, course_id: course, hashtags, attachments: attachments || [],
    author: new ObjectId(authorId),
    createdAt: new Date(), updatedAt: new Date(),
    upvote_count: 0, downvote_count: 0,
    upvoted_by: [], downvoted_by: [], comments: [], views: 0,
  }
...
async function handleVote(userId, post, voteType) {
  ...
  if (voteType === 'up') {
    if (alreadyUp) {
      await collection.updateOne({ _id: post._id }, { $inc: { upvote_count: -1 }, $pull: { upvoted_by: uid } })
    } else {
      if (alreadyDown) {
        await collection.updateOne({ _id: post._id }, { $inc: { downvote_count: -1 }, $pull: { downvoted_by: uid } })
      }
      if (post.author.toString() !== userId) {
        await xpService.addXP(post.author.toString(), 5, { totalLikesReceived: 1 })
      }
      await collection.updateOne({ _id: post._id }, { $inc: { upvote_count: 1 }, $push: { upvoted_by: uid } })
    }
  } else {
    if (alreadyDown) {
      await collection.updateOne({ _id: post._id }, { $inc: { downvote_count: -1 }, $pull: { downvoted_by: uid } })
      ...
      await collection.updateOne({ _id: post._id }, { $inc: { downvote_count: 1 }, $push: { downvoted_by: uid } })
    }
  }
}

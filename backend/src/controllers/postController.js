...
const getCommentReplies = async (req, res, next) => {
    try {
        ...
        const parentComment = postService.findCommentRecursive(
            post.comments,
...
const createPost = async (req, res, next) => {
    try {
        ...
        const insertedId = await postService.createPostDocument({
            title,
            content,
            course,
            hashtags,
        ...
        if (createdPost) {
            postService.addUserVoteStatus(createdPost, userId);
        }
...
const updatePost = async (req, res, next) => {
    try {
        ...
        if (updatedPost) {
            postService.addUserVoteStatus(updatedPost, req.user.id);
        }
...
const votePost = async (req, res, next) => {
    try {
        ...
        postService.addUrlsToItems(updatedPost ? [updatedPost] : []);
        if (updatedPost) {
            postService.addUserVoteStatus(updatedPost, userId);
        }
...
const addCommentToPost = async (req, res, next) => {
    try {
        ...
        if (parentId) {
            ...
            await mongoose.connection.db
                .collection('posts')
                .updateOne({ _id: new ObjectId(id) }, { $set: { comments: post.comments } });
        } else {
            await mongoose.connection.db
                .collection('posts')
...
const voteComment = async (req, res, next) => {
    try {
        ...
        const commentToVote = postService.findCommentRecursive(
            post.comments,
            new ObjectId(commentId),
        );
        if (!commentToVote) {
...
const bookmarkPost = async (req, res, next) => {
    try {
        ...
        const isBookmarked =
            ...
            user.bookmarked_posts.some((postId) => postId.equals(postObjectId));

        await User.updateOne(
            { _id: userId },
            isBookmarked
                ? { $pull: { bookmarked_posts: postObjectId } }
                : { $push: { bookmarked_posts: postObjectId } },
        );

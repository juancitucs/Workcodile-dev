...
const cursos: Curso[] = [
  ...
  {
    id: 'IS-123',
    nombre: 'METODOLOGIA Y TECNICAS DE ESTUDIO UNIVERSITARIO',
    ciclo: 1,
  },
  ...
  {
    id: 'IS-924',
    nombre: 'FORMACION DE EMPRESAS CON BASE TECNOLOGICA',
    ciclo: 9,
  },
  ...
  {
    id: 'IS-927',
    nombre: 'ELECTIVO I: PLANEAMIENTO ESTRATEGICO DE SISTEMAS DE INFORMACION',
    ciclo: 9,
  },
  ...
  { id: 'IS-1024', nombre: 'SEMINARIO DE TESIS', ciclo: 10 },
  {
    id: 'IS-1025',
    nombre:
      'ELECTIVO II: PLANEAMIENTO ESTRATEGICO DE TECNOLOGIA DE INFORMACION',
    ciclo: 10,
  },
...
const publicacionesMock: Publicacion[] = [
  {
    ...
    contenido:
      'Hola estudiantes de UNAM! Necesito ayuda con Matemática II, especialmente con integrales. Estoy dispuesto a pagar por sesiones de tutoría. Si alguien puede ayudarme, por favor contáctenme.',
    ...
    comentarios: [
      {
        id: '1',
        contenido:
          'Hola Ana! Yo puedo ayudarte con Matemática II. Soy estudiante de quinto ciclo de Ingeniería. Mándame un mensaje.',
  ...
  },
  {
    ...
    contenido:
      'Soy estudiante de Ingeniería de Sistemas y ofrezco servicios de desarrollo web (HTML, CSS, JavaScript, React). Precios accesibles para estudiantes. Portfolio disponible.',
    autor: usuariosMock[1],
    ...
    comentarios: [
      {
        id: '2',
        contenido:
          '¿Tienes experiencia con bases de datos? Necesito ayuda con un proyecto de MySQL.',
      ...
      },
      {
        id: '3',
        contenido:
          'Sí, manejo MySQL, PostgreSQL y MongoDB. Te puedo ayudar sin problema.',
        autor: usuariosMock[1],
  ...
  },
  {
    ...
    contenido:
      'Estoy formando un grupo de estudio para el curso de Estadística Básica del profesor Mamani. Nos juntaríamos los sábados por la tarde en la biblioteca. ¿Quién se apunta?',
    autor: usuariosMock[2],
  ...
  },
  {
    ...
    contenido:
      'Por mantenimiento, el laboratorio de computación estará cerrado del 20 al 25 de enero. Por favor planifiquen sus trabajos de Fundamentos de Programación con anticipación.',
    autor: usuariosMock[0],
    ...
    comentarios: [
      {
        id: '4',
        contenido:
          'Gracias por el aviso. ¿Hay algún laboratorio alternativo disponible?',
        autor: usuariosMock[1],
...
export function ProveedorApp({ children }: { children: ReactNode }) {
  ...
  const [publicaciones, setPublicaciones] =
    useState<Publicacion[]>(publicacionesMock)
  ...
  const buscarPublicaciones = (consulta: string) => {
    ...
    return publicaciones.filter(
      (publicacion) =>
        ...
        publicacion.hashtags.some((tag) =>
          tag.toLowerCase().includes(consulta.toLowerCase())
        )
    )
  }
  ...
  const calificarPublicacion = (
    ...
    setPublicaciones((prev) =>
      prev.map((publicacion) => {
        if (publicacion.id === idPublicacion) {
          const fueCalificadoPorUsuario =
            publicacion.calificacion_usuario &&
            publicacion.calificacion_usuario > 0
          const nuevoTotalCalificaciones = fueCalificadoPorUsuario
            ? publicacion.total_calificaciones
            : publicacion.total_calificaciones + 1

          // Calcular nueva calificación promedio
          const totalActual =
            publicacion.calificacion * publicacion.total_calificaciones
          ...
          const nuevaCalificacionPromedio =
            nuevoTotalCalificaciones > 0
              ? nuevoTotal / nuevoTotalCalificaciones
              : 0

          console.log(
            `Usuario ${usuario.nombre} calificó el post "${publicacion.titulo}" con ${calificacion} cocodrilos`
          )
  ...
  const reportarPublicacion = (idPublicacion: string) => {
    ...
    console.log(
      `Publicación ${idPublicacion} reportada por el usuario ${usuario?.id}`
    )
  }
